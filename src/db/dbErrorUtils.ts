...
export function getDbErrorMessage(error: unknown): { message: string; constraint: string | null } {
  ...
  if (error instanceof DrizzleQueryError && error.cause instanceof Error) {
    const missingPassword = "SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string"
    if (error.cause.message === missingPassword) {
      error.cause.message = `missing database url. Run "config set dbURL <url>" to set the database url.`
    }
    return { message: error.cause.message, constraint: null }
  }
